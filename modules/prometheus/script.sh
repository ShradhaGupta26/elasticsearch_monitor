...
cat << EOF | sudo tee /etc/grafana/provisioning/dashboards/elasticsearch.yml
apiVersion: 1
providers:
- name: 'elasticsearch'
  orgId: 1
  ...
  options:
    path:    /var/lib/grafana/elasticsearch.json
EOF
sudo yum install git -y

git clone https://github.com/ShradhaGupta26/Monitoring.git

cp /tmp/Monitoring/elasticsearch.json /var/lib/grafana/
